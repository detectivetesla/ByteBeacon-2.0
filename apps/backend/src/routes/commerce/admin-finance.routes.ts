    ...
    async (req, reply) => {
      ...
      try {
        ...
        if (action === 'APPROVE' || action === 'PROCESS') {
          ...
          await client.query(
            `UPDATE users SET wallet_balance_pesewas = COALESCE(wallet_balance_pesewas, 0) + $1, wallet_balance = ROUND((COALESCE(wallet_balance_pesewas, 0) + $1)::numeric / 100.0, 2), updated_at = CURRENT_TIMESTAMP WHERE id = $2`,
