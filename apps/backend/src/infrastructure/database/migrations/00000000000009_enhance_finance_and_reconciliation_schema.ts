...
export const migration00000000000009: MigrationFile = {
  version: '00000000000009',
  name: 'enhance_finance_and_reconciliation_schema',
  upSql: `
    ...
    CREATE TABLE IF NOT EXISTS financial_adjustments (
        ...
        requested_by UUID NOT NULL REFERENCES users(uuid) ON DELETE CASCADE,
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'refunds' AND column_name = 'requested_by') THEN
            ALTER TABLE refunds ADD COLUMN requested_by UUID REFERENCES users(uuid) ON DELETE SET NULL;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'refunds' AND column_name = 'approved_by') THEN
            ALTER TABLE refunds ADD COLUMN approved_by UUID REFERENCES users(uuid) ON DELETE SET NULL;
        END IF;
  ...
  `,
  downSql: `
    DROP TABLE IF EXISTS financial_safety_settings CASCADE;
    DROP TABLE IF EXISTS financial_adjustments CASCADE;
    DROP TABLE IF EXISTS reconciliation_cases CASCADE;
    ALTER TABLE refunds DROP COLUMN IF EXISTS reversal_type;
    ALTER TABLE refunds DROP COLUMN IF EXISTS risk_level;
    ALTER TABLE refunds DROP COLUMN IF EXISTS ledger_journal_id;
    ALTER TABLE refunds DROP COLUMN IF EXISTS admin_notes;
    ALTER TABLE refunds DROP COLUMN IF EXISTS approved_by;
    ALTER TABLE refunds DROP COLUMN IF EXISTS requested_by;
  `,
};
