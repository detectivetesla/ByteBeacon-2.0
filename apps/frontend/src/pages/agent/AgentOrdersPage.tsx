    ...
    try {
      ...
      if (res) {
        ...
        const mapped: OrderRowData[] = orderList.map((o: any) => {
          let resolvedOrderStatus: OrderStatus = OrderStatus.PROCESSING;
          ...
          if (rawStatus === 'APPROVED' || rawStatus === 'COMPLETED' || rawStatus === 'DELIVERED') {
            ...
            resolvedOrderStatus = OrderStatus.SUBMITTED;
          } else if (rawStatus === 'CANCELLED') {
            resolvedOrderStatus = OrderStatus.CANCELLED;
          } else if (rawStatus === 'PROCESSING') {
            resolvedOrderStatus = OrderStatus.PROCESSING;
          }

          let resolvedPaymentStatus: PaymentStatus = PaymentStatus.PENDING;
          const rawPay = (o.paymentStatus || '').toUpperCase();
          if (rawPay === 'PAID') resolvedPaymentStatus = PaymentStatus.PAID;
          else if (rawPay === 'FAILED') resolvedPaymentStatus = PaymentStatus.FAILED;
          else if (rawPay === 'REFUNDED') resolvedPaymentStatus = PaymentStatus.REFUNDED;
          else if (rawPay === 'PARTIALLY_REFUNDED') resolvedPaymentStatus = PaymentStatus.PARTIALLY_REFUNDED;
          else if (rawPay === 'PENDING') resolvedPaymentStatus = PaymentStatus.PENDING;
          ...
          const pesewas = typeof o.amountPesewas === 'number'
            ? o.amountPesewas
            : o.amount
  ...
  return (
    <div style={{ maxWidth: '1280px', margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      ...
      <section style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
        ...
        <LatestSuccessfulOrderBanner style={{ marginBottom: 'var(--space-2)' }} />
